...
\subsection{\acrshort{bleu} - bilingual evaluation understudy}
...
Combining all above, the metric works in this way (\cref{eq:bleu}):
\begin{enumerate}
...
The metric value is in range from 0 to 1, however, popular implementations
such as SacreBLEU \citep{Post2018-sacrebleu} report it in percent points
from 0 to 100.
...
Later we describe the specific approach from multi-target \acrshort{mt}
which we are using in this work.
